...
import { nonTechProjects } from "../Constants/constants";
...
const SectionWrapper = (Component, idName) => 
  ...
  const NonTechProjectsCard = ({ index, name, description, tags, image, source_code_link, demo_link }) => {
    return (
      <motion.div
      ...
      >
        <Tilt
          ...
          className='text-primary-400 p-5 rounded-lg sm:w-[280px] w-full ml-[-12]'
          style={{ marginTop: '-150%' }}
          >
          <div
  ...
  return (
    <>
      ...
      <div className=" flex flex-wrap gap-7">
        {nonTechProjects.map((nonTechProjects, index) => (
          <NonTechProjectsCard 
            key={`nonTechProjects-${index}`}
            index={index}
            {...nonTechProjects}
          />
        ))}
